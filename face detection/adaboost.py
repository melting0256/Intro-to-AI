...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        First,we set bestError as infinite.Second,we use two for loops to generate h(x) 
        and calculate each weightError (weight*|h(x)-label|),then we keep the minimum weightError
        as bestError.Use the index of this featureVal to find out feature and use this to get 
        bestClf.
        """
        bestError=float("inf")
        for i in range(0, len(featureVals)):
            weightedError=0.0
            for j in range(0, len(featureVals[i])):
                if featureVals[i][j]<0:
                    weightedError+=weights[j]*abs(1-labels[j])
                else:
                    weightedError+=weights[j]*abs(0-labels[j])
